...
MetricInfoArtifact = namedtuple("MetricInfoArtifact",
                                ["model_name", "model_object", "train_precision", "test_precision", "train_recall", "test_recall","train_accuracy",
                                 "test_accuracy","confusion_matrix", "classification_report","model_accuracy", "index_number"])
...
def evaluate_classification_model(model_list: list, X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray, base_accuracy: float = 0.6) -> MetricInfoArtifact:
    ...
    try:
        ...
        for index_number, model in enumerate(model_list):
            logging.info(f"model_object: {model}")
            ...
            confusion_matr=confusion_matrix(y_test,y_test_pred)
            ...
            logging.info(
                f"Classification report: {classification_report(y_test, y_test_pred)}"
            )
            logging.info(f"confusion matrix: {confusion_matrix(y_test, y_test_pred)}")
...
class ModelFactory:
    def __init__(self, model_config_path: str = None,):
        try:
            ...
            print(self.models_initialization_config)
