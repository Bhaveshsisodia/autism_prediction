...
from Autism.entity.config_entity import ModelEvaluationConfig
from Autism.entity.artifact_entity import DataIngestionArtifact,DataValidationArtifact,ModelTrainerArtifact,ModelEvaluationArtifact
...
from Autism.entity.model_factory import evaluate_classification_model
...
class ModelEvaluation:
    ...
    def get_best_model(self):
        try:
            ...
            if not os.path.exists(model_evaluation_file_path):
                write_yaml_file(file_path=model_evaluation_file_path,content=None
                                )
                return model
            model_eval_file_content = read_yaml_file(file_path=model_evaluation_file_path)

            model_eval_file_content = {} if model_eval_file_content is None else model_eval_file_content
    ...
    def update_evaluation_report(self, model_evaluation_artifact: ModelEvaluationArtifact):
        try:
            ...
            write_yaml_file(file_path=eval_file_path, content=model_eval_content)
        ...
        try:
            ...
            if metric_info_artifact is None:
                response = ModelEvaluationArtifact(is_model_accepted=False
                                                   ,message=f"Model {metric_info_artifact.model_name} is Not accepted",
                                                   evaluated_model_path=trained_model_file_path
                                                   )
    ...
    # evaluated_model_path : str
                ...
                model_evaluation_artifact = ModelEvaluationArtifact(evaluated_model_path=trained_model_file_path,
                                                                    message=f"Model {metric_info_artifact.model_name} is accepted",
                                                                    is_model_accepted=True)
                self.update_evaluation_report(model_evaluation_artifact)
                logging.info(f"Model accepted. Model eval artifact {model_evaluation_artifact} created")
